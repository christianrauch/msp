...
void Client::setVariant(const FirmwareVariant& v) {
    fw_variant = v;
}
...
bool Client::sendMessage(msp::Message& message, const double& timeout) {
    if (log_level_ >= DEBUG) std::cout << "sending message - ID " << message.id() << std::endl;
    if(!sendData(message.id(), message.encode())) { 
        if (log_level_ >= WARNING) std::cerr << "message failed to send" << std::endl;
        return false; 
    }
    ...
    const auto predicate = [&]{
        mutex_response.lock();
        const bool received = (request_received!=NULL) && (request_received->id==message.id());
        // unlock to wait for next message
        if(!received) { mutex_response.unlock(); }
        return received;
    };
    //depending on the timeout, we may wait a fixed amount of time, or indefinitely
    if(timeout>0) {
        if(!cv_response.wait_for(lock, std::chrono::milliseconds(size_t(timeout*1e3)), predicate)) {
            if (log_level_ >= INFO) std::cout << "timed out waiting for response to message ID " << message.id() << std::endl;
            return false;
        }
    } else {
...
bool Client::sendMessageNoWait(const msp::Message& message){
    if (log_level_ >= DEBUG) std::cout << "async sending message - ID " << message.id() << std::endl;
    if(!sendData(message.id(), message.encode())) { 
        if (log_level_ >= WARNING) std::cerr << "async sendData failed" << std::endl;
        return false; 
    }
    return true;
}
...
uint8_t Client::extractChar() {
    if(buffer.sgetc()==EOF) {
        if (log_level_ >= WARNING) std::cerr << "buffer returned EOF; reading char directly from port" << std::endl;
...
bool Client::sendData(const msp::ID id, const ByteVector &data) {
    ...
    if (log_level_ >= DEBUG) std::cout << "write complete: " << bytes_written << " vs " << msg.size() << std::endl;
...
void Client::processOneMessage(const asio::error_code& ec, const std::size_t& bytes_transferred) {
    if (log_level_ >= DEBUG) std::cout << "processOneMessage on " << bytes_transferred << " bytes" << std::endl;
    ...
    // check subscriptions
    {
        ...
        if(request_received->status==OK && subscriptions.count(ID(request_received->id))) {
            subscriptions.at(ID(request_received->id))->decode(request_received->payload);
        }
    }
    ...
    if (log_level_ >= DEBUG) std::cout << "processOneMessage finished" << std::endl;
}
...
ReceivedMessage Client::processOneMessageV2() {
    ...
    if(log_level_ >= WARNING && !ok_id) {
        std::cerr << "Message v2 with ID " << ret.id << " is not recognised!" << std::endl;
    }
    ...
    if(log_level_ >= WARNING && !ok_crc) {
        std::cerr << "Message v2 with ID " << ret.id << " has wrong CRC! (expected: " << size_t(exp_crc) << ", received: "<< size_t(rcv_crc) << ")" << std::endl;
    }
