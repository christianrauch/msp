...
public:
    ...
    /**
     * @brief Sets roll, pitch, yaw, and throttle values. They are sent immediately
     * and stored for resending if automatic MSP control is enabled.
     * @param rpyt An array of doubles representing roll, pitch, yaw, and throttle in that 
    ...
    /**
     * @brief Starts message to flight controller signalling that it should reboot. Will
     * result in the serial device disappearing if using a direct USB connection to the board.
     */
    bool reboot();
    
    /**
     * @brief Queries the currently set firmware variant (Cleanflight, Betaflight, etc.)
     ...
     * @param level LoggingLevel matching the desired amount of output (default to WARNING)
     */
    void setLoggingLevel(const msp::client::LoggingLevel& level);
    
    /**
     * @brief Set RC channels in order: roll, pitch, yaw, throttle by using channel mapping
     * @param roll
     * @param pitch
     * @param yaw
     * @param throttle
    ...
    /**
     * @brief Register callback function that is called when type is received
     * @param callback Function to be called (e.g. lambda, class method, function pointer)
     * @param tp Period of timer that will send subscribed requests (in seconds), by default this is 0 and requests are not sent periodically
     * @return Pointer to subscription that is added to internal list
     */
     ...
     * @param timeout Number of seconds to wait for response. Default value of 0 means no timeout.
     * @return True on success
     */
     ...
     * Not to be confused with armSet(), which queries whether the flight controller
     * has been instructued to turn on the ARM status.
     * @return True if the ARM status is active
     */
    bool isArmed() { return isStatusActive("ARM"); }

    /**
     * @brief Queries the flight controller to see if the FAILSAFE status is active.
