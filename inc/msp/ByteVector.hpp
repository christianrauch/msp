...
public:
    ...
    /**
     * @brief ByteVector constructor
     * @param arg1 Single argument passed to constructor of parent std::vector class
     */
    ...
    /**
     * @brief ByteVector constructor
     * @param arg1 First of two arguments passed to constructor of parent std::vector class
     * @param arg2 Second of two arguments passed to constructor of parent std::vector class
     */
    ...
    /**
     * @brief Packs integer types into the ByteVector. Ensures little endian packing.
    ...
    /**
     * @brief Packs floating point types into the ByteVector. Uses IEEE 754 format.
     * @tparam T Underlying data type to be packed. Must be a floating point type.
     * @param val Value to be packed
    ...
    /**
     * @brief Packs scaled values (e.g. float to scaled int) as packed_val = (val+offset)*scale
     * @tparam encoding_T Data type to use for the actual data packing (usually an integral type)
     ...
     * @param offset Value of offset to apply to the input value (optional, defaults to 0)
     * @return True if successful
     */
     ...
     * @param max_len Optional max number of characters to transfer into the ByteVector
     * @return True if successful
     */
    bool pack(const std::string& val, size_t max_len = std::numeric_limits<size_t>::max()) {
        ...
        // definitly correct in tested cases
        this->push_back(0);
        return true;
    }
    ...
    /**
     * @brief Packs an an object which inherits from type Packable into the ByteVector
    ...
    /**
     * @brief Packs scaled value types (e.g. value<float> to scaled int) as packed_val = (val+offset)*scale
     * @tparam encoding_T Data type to use for the actual data packing (usually an integral type)
     * @tparam T1 Type of input value (usually a floating point type)
     * @tparam T2 Type of scale and offset coefficients
     * @param val Value to be packed
     ...
     * @tparam T Type of the Value<T> being packed. May be automatically deduced from arguments
     ...
     * bytes are available.
     * @tparam T Underlying data type to be extracted. Must be a floating point type.
     * @param val Destination of unpack operation. 
     * @return True on successful unpack
     */
    template<typename T, typename std::enable_if<std::is_floating_point<T>::value, T>::type* = nullptr>
    ...
    /**
     * @brief Extracts data from the ByteVector and stores it in a
     * another ByteVector. Consumes all remaining data unless instructed otherwise.
     * @param val Destination of unpack operation. 
     * @param count Max number of bytes to extract. Optional, if unset, all 
     * remaining bytes will be consumed.
     * @return True on successful unpack
    ...
    /**
     * @brief Unpacks scaled value types (e.g. scaled int to floating point) as val = (packed_val/scale)-offset
     * @tparam encoding_T data type used to store the scaled value (usually an integral type)
    ...
    /**
     * @brief Unpacks Value types other than string and ByteVector specializations
     * @tparam T Type of the Value<T> being packed. May be automatically deduced from arguments
    ...
    /**
     * @brief Extracts data from the ByteVector and stores it in a
     * Value<std::string>. Consumes all remaining data unless instructed otherwise.
     * @param val Destination of unpack operation. 
     * @param count Max number of bytes to extract. Optional, if unset, all 
     * remaining bytes will be consumed.
     * @return True on successful unpack
    ...
    /**
     * @brief Extracts data from the ByteVector and stores it in a
     * Value<ByteVector>. Consumes all remaining data unless instructed otherwise.
     * @param val Destination of unpack operation. 
     * @param count Max number of bytes to extract. Optional, if unset, all 
     * remaining bytes will be consumed.
     * @return True on successful unpack
